...
class KsData():
	def __init__(self, data_path="data/test_10.txt", item_number=10):
		np.random.seed(0)
		data = []
		labels = []
		...
		with open(data_path) as file:
			for line in file:
				...
				# data_3 = [float(i) for i in tokens[1].split(", ")]
				train_data = data_1
...
def gen_weakconstraint():
	...
	with open("data/evidence_train.txt", "w") as f:
		for index in range(len(ks.train_data)):
			...
			# 	f.write(value_begin+str(i)+","+str(int(train_value[index][i]))+").\n")
				# f.write(weight_begin+str(i)+","+str(int(train_weight[index][i]))+").\n")
			for i in range(ks.item_number):	
	...
	with open("data/evidence_valid.txt", "w") as f:
		for index in range(len(ks.valid_data)):
			...
			f.write(begin2+"\n")
			# for i in range(ks.item_number):	
			# 	f.write(value_begin+str(i)+","+str(int(valid_value[index][i]))+").\n")
				# f.write(weight_begin+str(i)+","+str(int(train_weight[index][i]))+").\n")
			for i in range(ks.item_number):	
	...
	with open("data/evidence_test.txt", "w") as f:
		for index in range(len(ks.test_data)):
			...
			f.write(begin2+"\n")
			# for i in range(ks.item_number):	
			# 	f.write(value_begin+str(i)+","+str(int(test_value[index][i]))+").\n")
				# f.write(weight_begin+str(i)+","+str(int(train_weight[index][i]))+").\n")
			for i in range(ks.item_number):	
